...
echo "version check says moi!"
echo "${ECR_REPO}/${ARTIFACT_NAME}:ci-${TRAVIS_BUILD_NUMBER}"

#if [ "${TRAVIS_EVENT_TYPE}" = "cron" ]
#then
docker run -v ${TRAVIS_BUILD_DIR}/variants/fatjar-openjdk8:/repository "${ECR_REPO}/baseimage-fatjar-jdk8:ci-${TRAVIS_BUILD_NUMBER}" /bin/sh -c "apk info -v | sort > /repository/package-versions && chmod 755 /repository/package-versions"

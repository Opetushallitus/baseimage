...
if [[ "X${DONT_INCLUDE_LOGBACK_CONFIG}" == X && ! "${NAME}" =~ ^ovara ]]; then
    ...
    elif [ ${NAME} == "oma-opintopolku" ] || [ ${NAME} == "ohjausparametrit" ] || [ ${NAME} == "valintalaskenta-ui" ] || [ ${NAME} == "valintaperusteet-ui" ] || [ ${NAME} == "valintaperusteet-service" ] || [ ${NAME} == "valintalaskentakoostepalvelu" ] || [ ${NAME} == "lokalisointi" ]; then
	echo "${NAME}"
    else
